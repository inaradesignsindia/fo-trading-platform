...
app.post('/api/brokers/:brokerName', async (req, res) => {
    ...
    const { apiKey, apiSecret, userId, vendorCode, redirectUri, authToken } = req.body;
...
app.post('/api/brokers/:brokerName/test', async (req, res) => {
    ...
    setTimeout(() => {
        ...
        if (isSuccess) {
            ...
            res.json({ success: true, message: `${brokerName} connection successful` });
        } else {
            configs.brokers[brokerName] = {
                ...configs.brokers[brokerName],
                status: 'error',
...
app.post('/api/ai/analyze/free', async (req, res) => {
    ...
    try {
        ...
        const mockAnalysis = {
            ...
            signals: [
                { indicator: 'RSI', value: Math.floor(30 + Math.random() * 40), signal: 'Neutral' },
                { indicator: 'MACD', value: (Math.random() - 0.5) * 10, signal: Math.random() > 0.5 ? 'Buy' : 'Sell' }
            ],
...
app.listen(PORT, async () => {
    ...
    console.log('🔧 Free integrations: Yahoo Finance, Alpha Vantage, TradingView Basic');
