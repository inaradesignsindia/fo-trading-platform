...
const icons = {
  ...
  Plug: (props) => (
    ...
    >
      ...
      <path d="M10 17l4-2" />
      <path d="M14 17l-4-2" />
      <path d="M18 10a6 6 0 1 0-12 0v5h12v-5z" />
      <path d="M5 22h14" />
    </svg>
  ),
  Activity: (props) => (
...
const mockIndices = [
  {
    ...
    price: 18540.75,
    change: 75.15,
    chartData: [4, 5, 5, 6, 7, 8, 7],
  },
  {
    symbol: 'BANK NIFTY',
    price: 43890.25,
    change: -120.5,
    chartData: [8, 7, 6, 6, 5, 4, 5],
  },
  {
    symbol: 'FIN NIFTY',
    price: 19450.6,
...
const mockPositions = [
  { contract: 'NIFTY 18600 CE', qty: 50, avg: 110.5, ltp: 115.8 },
  { contract: 'BANKNIFTY 43800 PE', qty: 15, avg: 250.0, ltp: 220.1 },
];
const mockAiAnalysis = {
  sentiment: 'Slightly Bullish',
  pcr: 1.15,
  maxPain: 18500,
  summary: 'PCR > 1 suggests puts are being written, providing market support.',
};
const mockWatchlist = ['NIFTY 18700 CE', 'BANKNIFTY 44000 CE', 'RELIANCE'];
const mockRecentTrades = [
  { type: 'BUY', contract: 'FINNIFTY 19500 CE', qty: 40, price: 80.2 },
];
const mockAiSuggestion = {
  ...
  target: 190.0,
  confidence: 'High (85%)',
  rationale: 'Nifty is showing strong upward momentum.',
};
...
const Sidebar = ({ activePage, setActivePage }) => (
  <aside className="w-20 bg-[#161B22] p-4 flex flex-col items-center justify-between border-r border-gray-800">
    <div>
      <div className="w-10 h-10 bg-blue-600 rounded-lg mb-10 grid place-items-center font-bold text-xl text-white">
        F
      </div>
      <nav className="space-y-6">
        {/* AI Quant Analysis Page - Now First */}
        <button
          onClick={() => setActivePage('ai-quant')}
          className={`block p-2 rounded-lg transition-colors ${activePage === 'ai-quant' ? 'bg-blue-700/50 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}
        >
          <icons.Brain />
        </button>
        {/* Unified Trading Page (One-Touch + Scalper combined) */}
        <button
          onClick={() => setActivePage('unified-trading')}
          className={`block p-2 rounded-lg transition-colors ${activePage === 'unified-trading' ? 'bg-blue-700/50 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}
        >
          <icons.Zap />
        </button>
        <button
          onClick={() => setActivePage('dashboard')}
          className={`block p-2 rounded-lg transition-colors ${activePage === 'dashboard' ? 'bg-blue-700/50 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}
        >
          <icons.LayoutDashboard />
        </button>
        <button
          onClick={() => setActivePage('option-chain')}
          className={`block p-2 rounded-lg transition-colors ${activePage === 'option-chain' ? 'bg-blue-700/50 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}
        >
          <icons.List />
        </button>
      </nav>
    ...
    <div className="space-y-6">
      ...
      <button
        ...
        className={`block p-2 rounded-lg transition-colors ${activePage === 'settings' ? 'bg-blue-700/50 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}
      >
        <icons.Settings />
      </button>
      <button
        className="block p-2 text-gray-500 hover:text-red-500 transition-colors"
  ...
  <header className="flex justify-between items-center pb-4 border-b border-gray-800 mb-6">
    <div>
      ...
      <p className="text-sm text-gray-400">
        Welcome back, Trader! Deploying low-latency strategies.
      </p>
    </div>
...
const BrokerAPISettingsPanel = () => {
  const brokers = [
    ...
    },
    {
      ...
      status: 'Pending Auth',
      color: 'yellow-500',
      fields: ['Auth Token', 'Websocket Key'],
    },
  ];
  ...
  return (
    <div className="glass-card p-6 rounded-xl space-y-6">
      ...
      <p className="text-gray-400">
        Manage connections to your supported trading accounts. Use the 'Connect'
        button to configure credentials or initiate OAuth.
      </p>
      ...
      <div className="space-y-4">
        {brokers.map((broker) => (
          <div
            key={broker.name}
            className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-gray-900 rounded-lg border border-gray-800 hover:border-blue-600 transition-colors"
          >
            <div className="flex items-center gap-4 mb-2 sm:mb-0">
              <div
                className={`w-10 h-10 rounded-full bg-blue-800 grid place-items-center font-bold text-lg text-white`}
              >
                {broker.logo}
              </div>
              <div>
                ...
                <div className="flex items-center text-sm gap-2 mt-1">
                  <span
                    className={`w-2 h-2 rounded-full bg-${broker.color}`}
                  ></span>
                  <span className={`text-${broker.color} font-medium`}>
                    {broker.status}
                  </span>
                </div>
              </div>
    ...
    charting: [
      {
        ...
        description:
          'Advanced charting with indicators, alerts, and replay features',
      ...
      },
      {
        ...
        description:
          'Essential charting with basic indicators and limited features',
        status: 'Available',
        color: 'green-500',
        action: 'Enable Free',
    ...
    <div className="space-y-6">
      <div className="glass-card p-6 rounded-xl">
        ...
        <p className="text-gray-400 mb-6">
          Configure all external services required for live trading
        </p>
        ...
        <div className="space-y-4">
          {integrations[activeSection]
            ?.filter(
              (integration) =>
                tierFilter === 'all' ||
                integration.tier === tierFilter ||
                (tierFilter === 'premium' &&
                  ['premium', 'custom'].includes(integration.tier))
            )
            .map((integration) => (
              <div
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-2 h-2 rounded-full bg-${integration.color}`}
                        ></span>
                        <span
                          className={`text-${integration.color} font-medium text-sm`}
                        >
                          {integration.status}
                        </span>
                      </div>
                  ...
                  <button
                    className={`mt-3 sm:mt-0 font-medium py-2 px-4 rounded-md transition-colors ${
                      integration.tier === 'free'
  ...
  <div className="glass-card p-6 rounded-xl space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input
          type="email"
          defaultValue="john.doe@trade.com"
          disabled
          className="w-full bg-gray-800 border border-gray-700 rounded-md p-3 text-gray-500"
        />
      </div>
    </div>
    ...
    <div className="pt-4 border-t border-gray-800 mt-6">
      <p className="text-sm text-gray-500">
        For security reasons, any change in API configuration requires
        re-authentication.
      </p>
    </div>
  </div>
    ...
    <div className="flex-1 min-w-[300px]">
      <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-2">
        <span className="text-lg font-bold text-white">
          {type} 07OCT {data.strike} {type === 'Call' ? 'CE' : 'PE'}
        </span>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          className={`bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg shadow-xl transition-transform transform hover:scale-[1.02]`}
        >
          Sell {type}
        </button>
        <button
          className={`bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg shadow-xl transition-transform transform hover:scale-[1.02]`}
        >
          Buy {type}
        </button>
      </div>
    ...
    <div className="mt-6">
      ...
      <div className="glass-card p-6 rounded-xl">
        ...
        <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg mb-6 text-sm">
          ...
          <button className="bg-blue-600 px-3 py-1 rounded text-white text-xs">
            Show Chain
          </button>
        </div>
...
const AIQuantAnalysisPage = () => {
  ...
  const aiRecommendations = {
    NIFTY: {
      ...
      rationale:
        'Strong momentum breakout with RSI divergence. Volume confirmation at key support levels.',
      pcr: 1.15,
      maxPain: 18500,
      vix: 14.2,
    ...
    BANKNIFTY: {
      ...
      rationale:
        'Banking sector showing weakness. FII selling pressure and credit concerns.',
      pcr: 0.85,
      maxPain: 43800,
      vix: 16.8,
    ...
    FINNIFTY: {
      ...
      rationale:
        'Consolidation phase. Low volatility suggests range-bound movement.',
      pcr: 1.05,
      maxPain: 19450,
      vix: 13.5,
      sentiment: 'Neutral',
        ...
        <div className="h-80 bg-gray-900 rounded-lg flex flex-col items-center justify-center">
          <div className="text-center text-gray-400">
            ...
            <div className="text-sm">
              Live {symbol} chart with AI indicators
            </div>
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${signalBg} text-white`}
              >
            ...
            <div className="glass-card p-4 rounded-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                ...
                <div className="bg-gray-900 p-3 rounded-lg">
                  ...
                  <div
                    className={`font-bold ${
                      currentReco.signal === 'BULLISH'
                        ? 'text-green-400'
                        : currentReco.signal === 'BEARISH'
                          ? 'text-red-400'
                          : 'text-yellow-400'
                    }`}
                  >
                ...
                <div className="bg-gray-900 p-3 rounded-lg">
                  ...
                  <div className="text-white font-bold">
                    {currentReco.confidence > 80
                      ? 'LOW'
                      : currentReco.confidence > 60
                        ? 'MODERATE'
                        : 'HIGH'}
                  </div>
                </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-blue-200/80">
              Charts powered by TradingView's free widgets with real-time data
              for NIFTY, BANK NIFTY, and FIN NIFTY. AI analysis combines
              technical indicators, options flow, and quantitative models for
    ...
    <>
      ...
      <div className="mb-6 p-4 bg-green-900/20 border border-green-700/30 rounded-xl">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() =>
              setChartType(chartType === 'free' ? 'premium' : 'free')
            }
              ...
              <div className="h-96 bg-gray-900 rounded-lg flex flex-col items-center justify-center">
                <div className="text-gray-400 text-center">
                  ...
                  <div className="text-sm mt-1">
                    Open-source TradingView charting library
                  </div>
  ...
  <div className="mt-6">
    ...
    <div className="glass-card p-4 rounded-xl h-[700px] grid place-items-center">
      <p className="text-center text-gray-500">
        The full Option Chain table (CE/PE/IV/OI/Greeks) will be displayed here.
      </p>
    </div>
  </div>
);
...
const TradeTabPanel = ({ data, type }) => {
  ...
  const currentHeaders =
    type === 'positions' ? headers.positions : headers.orders;
    ...
    <div className="overflow-x-auto">
            ...
            >
              {type === 'positions' && (
                <>
                  ...
                  <td
                    className={`p-2 font-semibold ${item.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}
                  >
                    {item.pnl.toFixed(2)}
                  </td>
                </>
              ...
              {type === 'orders' && (
                <>
                  ...
                  <td
                    className={`p-2 font-bold ${item.status === 'EXECUTED' ? 'text-green-400' : item.status === 'OPEN' ? 'text-yellow-400' : 'text-red-400'}`}
                  >
                    {item.status}
                  </td>
                </>
      ...
      {data.length === 0 && (
        <p className="text-center text-gray-500 p-4">
          No {type === 'positions' ? 'open positions' : 'orders'} found.
        </p>
      )}
    </div>
  );
...
const OneTouchTradePage = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'Trade Book':
        return (
          <p className="text-gray-500 p-4">Trade Book data will appear here.</p>
        );
      case 'Holdings':
        return (
          <p className="text-gray-500 p-4">
            Equity and F&O Holdings summary here.
          </p>
        );
      case 'Funds':
        return (
          <p className="text-gray-500 p-4">
            Fund details and margin utilization here.
          </p>
        );
      default:
        return null;
      ...
      <div className="glass-card p-6 rounded-xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 text-sm mb-6">
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <div className="flex gap-1">
              <select
                className="w-1/3 bg-gray-900 border border-gray-700 rounded-l-md p-2 text-white"
                value={strikeType}
          ...
          <div>
            ...
            <input
              ...
              min="1"
              className="w-full bg-gray-900 border border-gray-700 rounded-md p-2 text-white focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
          ...
          {isComplexStrategy && strategy === 'COVER' && (
        ...
        <div className="border border-gray-700 p-4 rounded-xl space-y-4">
          <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg flex-wrap">
            <div className="flex items-center gap-4">
              ...
              <p className="text-sm">
                LTP: 798.80{' '}
                <span className="text-green-400">(77.85 / 10.80%)</span>
              </p>
            </div>
            <div className="flex items-center gap-2 mt-2 sm:mt-0">
              <button className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded-md shadow-lg">
                Sell Call
              </button>
              <button className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-md shadow-lg">
                Buy Call
              </button>
            </div>
          </div>
          ...
          <div className="text-center my-4">
            ...
            <div className="flex justify-center gap-4 mt-3 flex-wrap">
              <button className="bg-gray-700 hover:bg-red-700 text-white font-bold py-2 px-6 rounded-md transition-colors text-sm">
          ...
          <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg flex-wrap-reverse">
            ...
            <div className="flex items-center gap-4 text-right">
              <p className="text-sm">
                LTP: 441.00{' '}
                <span className="text-red-400">(-128.75 / -22.60%)</span>
              </p>
        ...
        <div className="mt-6">
          ...
          <div className="flex border-b border-gray-800 mb-4 text-sm overflow-x-auto">
            {['Positions', 'Order Book', 'Trade Book', 'Holdings', 'Funds'].map(
              (tab) => (
                <button
                  key={tab}
          ...
          <div className="bg-gray-900 rounded-lg">{renderTabContent()}</div>
        </div>
      </div>
    </div>
  );
        ...
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <p
                className={`text-2xl font-mono font-bold mt-2 ${index.change >= 0 ? 'text-green-400' : 'text-red-400'}`}
              >
  ...
  <div className="glass-card p-4 rounded-xl h-[500px] mb-6">
    <div className="flex justify-between items-center mb-4">
      ...
      <div className="flex items-center gap-2 text-sm">
        <button className="px-3 py-1 bg-blue-600 text-white rounded-lg shadow-md">
          1D
        </button>
        <button className="px-3 py-1 bg-gray-900 rounded-lg text-gray-400 hover:bg-gray-700/50">
          5D
        </button>
        <button className="px-3 py-1 bg-gray-900 rounded-lg text-gray-400 hover:bg-gray-700/50">
          1M
    ...
    <div className="w-full h-[calc(100%-40px)] bg-gray-900 rounded grid place-items-center">
      <p className="text-gray-600">
        TradingView Chart Library Would Be Integrated Here
      </p>
    </div>
  </div>
);
...
const PositionsPanel = ({ positions }) => {
  const totalPL = positions.reduce(
    (acc, pos) => acc + (pos.ltp - pos.avg) * pos.qty,
    0
  );
  return (
    <div className="glass-card p-4 rounded-xl">
      <div className="flex justify-between items-center mb-4">
        ...
        <div>
          ...
          <span
            className={`font-semibold ${totalPL >= 0 ? 'text-green-400' : 'text-red-400'}`}
          >
            {totalPL.toFixed(2)}
          </span>
        </div>
  ...
  <div className="glass-card p-4 rounded-xl">
    ...
    <div className="mt-4 space-y-2 text-sm">
      ...
      <div className="flex justify-between">
        ...
        <span
          className={`font-semibold ${portfolio.unrealizedPL >= 0 ? 'text-green-400' : 'text-red-400'}`}
        >
...
const AiAnalysisPanel = ({ analysis }) => (
  <div className="glass-card p-4 rounded-xl">
    ...
    <p className="text-xs text-gray-400 bg-gray-900 p-2 rounded-md">
      {analysis.summary}
    </p>
  </div>
);
      ...
      <div className="text-sm space-y-2">
        {activeTab === 'watchlist' &&
          watchlist.map((item) => (
            <div
              key={item}
              className="flex justify-between p-1 hover:bg-gray-800/50 rounded-md cursor-pointer"
            >
        ...
        {activeTab === 'trades' &&
          trades.map((trade, i) => (
            ...
            >
              <div>
                <span
                  className={`font-bold ${trade.type === 'BUY' ? 'text-green-400' : 'text-red-400'}`}
                >
                  {trade.type}{' '}
                </span>
...
const UnifiedTradingPage = () => {
  ...
  const priceChange = 241.3;
  const percentChange = 0.44;

  // Mock LTP data for call and put options
  const callLTP = { price: 798.8, change: 77.85, percent: 10.8 };
  const putLTP = { price: 441.0, change: -128.75, percent: -22.6 };
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'Order book':
        return (
          <div className="text-center text-gray-400 p-8">No pending orders</div>
        );
      case 'Trade Book':
        return (
          ...
          <div className="flex items-end">
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor="oneClick" className="text-gray-400 text-sm">
                One click: {oneClickDisabled ? 'Disable' : 'Enable'}
              </label>
            </div>
          </div>
        </div>
      ...
      <div className="glass-card p-6 rounded-xl mb-6">
        ...
        <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg mb-4">
          <div className="flex items-center gap-4">
            ...
            <p className="text-sm">
              ...
              <span
                className={`ml-2 ${callLTP.change >= 0 ? 'text-green-400' : 'text-red-400'}`}
              >
                ({callLTP.change >= 0 ? '+' : ''}
                {callLTP.change} / {callLTP.percent >= 0 ? '+' : ''}
                {callLTP.percent}%)
              </span>
            </p>
          </div>
        ...
        <div className="text-center my-6">
          <p className="text-4xl font-bold text-white">
            ...
            <span
              className={`ml-4 text-lg ${priceChange >= 0 ? 'text-green-400' : 'text-red-400'}`}
            >
              ({priceChange >= 0 ? '+' : ''}
        ...
        <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg">
          ...
          <div className="flex items-center gap-4">
            <p className="text-sm">
              ...
              <span
                className={`ml-2 ${putLTP.change >= 0 ? 'text-green-400' : 'text-red-400'}`}
              >
                ({putLTP.change >= 0 ? '+' : ''}
                {putLTP.change} / {putLTP.percent >= 0 ? '+' : ''}
                {putLTP.percent}%)
              </span>
            </p>
